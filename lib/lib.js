...
function collectImg(data, filename ){
    var result = {};
    result[filename] = data;

    for(var i = 0 ; i <data.length;i++){
        result[filename][i].imgsrc = `/static${result[filename][i].imgsrc}`;
    }
...
exports.download = function (filename) {
    ...
    if (specific_group) {
        downloaded = specific_group(downloaded[filename],filename);
    }
...
exports.delete = function (filename, data) {
    var id = data.id;
    var sql = sqlProcedures.delete({ filename, id });
    var result = connection.query(sql);
    if(result.affectedRows){
        return {message:"Data have been sucessfully deleted from database", rows:result.affectedRows};
    }else{
        throw new Error("Unreacheable data detected. Please check id value and try again");
    }   
}

exports.update = function(filename, data){
    var id = data.id;
    var column = data.column;
    var value = data.value;
    var sql = sqlProcedures.update({ filename, id, column, value });
    console.log(sql);
    var result = connection.query(sql);
    
    console.log(result);
    if(result.affectedRows){
        return {message:"Data have been sucessfully updated", rows:result.affectedRows};
    }else{
        throw new Error("Unreacheable data detected. Please check id value and try again");
    }   
}